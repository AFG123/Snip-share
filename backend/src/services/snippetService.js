...
async function createSnippet({
  ...
  const resolvedExpiryHours = Number.isFinite(Number(expiryHours)) ? Number(expiryHours) : Number(expiry);
  const expiryAt = new Date(Date.now() + resolvedExpiryHours * 60 * 60 * 1000);
